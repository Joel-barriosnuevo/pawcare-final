    ...
    try {
      ...
      if (error.response) {
        if (error.response.status === 403) {
          setError("No tienes permiso para realizar esta acción. Por favor, verifica tus credenciales.")
        } else if (error.response.status === 400) {
          setError(error.response.data.message || "Datos de registro inválidos. Por favor, verifica la información.")
        } else {
          setError(error.response.data.message || "Error al crear la cuenta. Por favor, intenta nuevamente.")
        }
